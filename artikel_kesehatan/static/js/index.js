const getData = (data) => {
	...
	data.forEach((item) => {
		...
		content += `
			<div id="card-${item.pk}" class="p-6 w-96 m-2 hover:scale-105 duration-200 max-w-sm bg-cream-muda rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
				...
				<div class="flex flex-row justify-between items-center">
					...
					<button onClick="deleteData(${item.pk})" class="text-sm sm:text-md hover:cursor-pointer bg-merah-tua hover:bg-merah-muda hover:bg-light_blue text-blue font-semibold hover:text-blue py-2 px-2 border border-cream-tua hover:border-cream-tua rounded">
...
const deleteData = (id) => {
	let csrftoken = $("input[name=csrfmiddlewaretoken]").val();
	$.ajax({
		url: `/artikel-kesehatan/hapus/${id}`,
		type: "DELETE",
		headers: {
